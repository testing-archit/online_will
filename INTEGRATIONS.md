...
| Speech | `POST /api/speech/synthesize` (`language: en` → 24 kHz WAV, Deepgram Aura-2; `hi`/`hinglish` → WAV, Smallest.ai Lightning v3.1 Pro), `POST /api/speech/listen?language=auto\|en\|hi\|hinglish` (raw audio → transcript, Deepgram Nova-3, nothing stored) |
| AI | `POST /api/assistant/extract`, `/api/interview/respond`, `/api/copilot/answer`, `/api/speech/transcribe`, `/api/documents/analyze`, `/api/documents/search`, `/api/legal/answer`, `/api/execution/analyze` (rate limited) |
...
Voice: Samaira replies in the language you speak (auto-detected per turn: English, Hindi in Devanagari, or Hinglish in Roman script; you can pin one). English is voiced by Deepgram Aura-2; Deepgram has no Hindi voice, so Hindi/Hinglish are voiced by Smallest.ai (`SMALLEST_API_KEY`, voice `meher`) and fall back to the best matching browser voice if it is unset or fails. Repeated lines are served from a bounded in-memory cache (`SMALLEST_TTS_CACHE_MB`, default 64, `0` disables) so they cost no extra API calls; it is never written to disk. Smallest.ai retains submitted text by default (auto-deletion is Enterprise-only), so review their DPA before relying on it for real client data. Listening uses Nova-3 `multi` mode so Hindi/English code-switching works. Audio sent for transcription goes to Deepgram and is not stored by this server. If `DEEPGRAM_API_KEY` is missing everything falls back to the browser.
