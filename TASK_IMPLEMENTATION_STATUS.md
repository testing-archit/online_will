...
23–24. 🟡 Voice and Hindi/Hinglish interview: **"Talk to Samaira" conversation mode** — hands-free loop (she speaks, listens, replies, listens again; "stop" ends it; two silent turns pause it). **Language is automatic**: each turn is classified as English, Hindi or Hinglish and she answers in that style. Ears: Deepgram Nova-3 (`multi`) with automatic end-of-speech detection, else browser recognition, else record-and-tap. Voice: Deepgram Aura-2 for English (sentence-by-sentence, ~1–2 s to first audio), best-ranked browser voice for Hindi/Hinglish. Confirmation is never spoken — proposals appear on screen. Verified live against Deepgram and Gemini with real audio (English, Hindi, Hinglish round trips) and in tests with fake speech APIs; **not** verified with a real microphone in a browser, and Hindi/Hinglish *speech output* quality depends on the voices installed on the device.
